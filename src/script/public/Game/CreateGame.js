...
const InitCreateLevelScene = () => {
    ...
    CreatePattern_createPattern_btn.addEventListener("click", () => {
        ...
        if (createCostumPattern_title.textContent != "pattern name" && drawed) {
...
const createCell = (x, index, grid, eventListener) => {
    ...
    let Cell = ConfigureCellSize(cell, x);

    return Cell;
};

// update cell. toggle its content
const createField_updateCell = (i, parentGrid) => {
    ...
    if (cell.classList.contains("draw")) { // cell is full. empty it
        ...
        if (userInfoClass != "empty") {
            cell.className = `cell ${userInfoClass}`;
